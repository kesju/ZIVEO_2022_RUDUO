...
from zive_util import read_seq
...
def create_sets_from_rec(rec_dir, SCHEMA_PATH, sets_path, wl_side, wr_side):
  # Suformuoja mokymo ir vertinimo sekų masyvus iš EKG įrašų.
  # Sekos formuojamos panaudojant aplanke SCHEMA_PATH saugomą atrinktų pūpsnių
  # indeksų masyvius train_ind ir validate_ind ir aplanke rec_dir saugomus EKG įrašus
  # bei visų EKG įrašuose esančių pūpsnių atributų masyvą all_beats_attr

  # Nuskaitome all_beats_attr
  file_path = Path(rec_dir, 'all_beats_attr.csv')
  all_beats_attr = pd.read_csv(file_path, index_col=0)

  # Nuskaitome mokymo imties sekų indeksų sąrašus ir suformuojame mokymo imtį su klasių numeriais
  file_path = Path(SCHEMA_PATH, 'train_ind_lst.csv')
  train_ind = list(np.loadtxt(file_path, delimiter=',', dtype="int"))
  X_train, y_train = create_set_from_rec(rec_dir, all_beats_attr, train_ind, wl_side, wr_side)

  # Įrašome mokymui skirtus masyvus į npz file diske
  file_path = Path(sets_path, 'train_set.npz')
  np.savez_compressed(file_path, X=X_train, y=y_train)
  # print("\nMokymo sekų ir jų klasių numerių masyvas įrašytas: į \n", file_path )

  # Nuskaitome vertinimo imties sekų indeksų sąrašus ir suformuojame vertinimo imtį su klasių numeriais
  file_path = Path(SCHEMA_PATH, 'validate_ind_lst.csv')
  validate_ind = list(np.loadtxt(file_path, delimiter=',', dtype="int"))
  X_validate, y_validate = create_set_from_rec(rec_dir, all_beats_attr, validate_ind, wl_side, wr_side)

  # # Įrašome vertinimui skirtus masyvus į npz file diske
  file_path = Path(sets_path, 'validate_set.npz')
  np.savez_compressed(file_path, X=X_validate, y=y_validate)
  # print("\nVertinimo sekų ir jų klasių numerių masyvas įrašytas: į \n", file_path )


def get_sets(sets_path):
...
def create_set_from_rec(rec_dir, all_beats_attr, ind_lst, window_left_side, window_right_side):
    ...
    count = 0
    for idx in ind_lst:
        seq_1d, sample, label = read_seq(rec_dir, all_beats_attr, idx, window_left_side, window_right_side)
        if (label != None):
            X[count,:] = seq_1d 
            y[count] = label
            count +=1
